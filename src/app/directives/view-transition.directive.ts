...
export class ViewTransitionDirective implements OnInit, OnDestroy {
  ...
  @Input() viewTransition: string = 'default';
  ...
  ngOnInit() {
    ...
    this.renderer.setStyle(element, 'transition', `all ${this.transitionDuration}ms cubic-bezier(0.4, 0, 0.2, 1)`);
  ...
  private animateIn(element: HTMLElement) {
    ...
    this.renderer.setStyle(element, 'transform', 'translateY(10px) scale(0.98)');
