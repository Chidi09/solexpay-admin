...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RippleDirective, ShakeOnErrorDirective],
  template: `
        ...
        <div class="bg-surface-container-lowest rounded-2xl p-8 shadow-[0_8px_40px_rgba(25,28,29,0.08)]">
          ...
          @if (success()) {
            <div class="text-center space-y-4">
              <div class="w-14 h-14 rounded-full bg-[color:var(--md-sys-color-secondary-container)] flex items-center justify-center mx-auto">
                <span class="material-symbols-outlined text-[color:var(--md-sys-color-on-secondary-container)] text-3xl">check_circle</span>
              </div>
              <h2 class="text-xl font-bold text-on-surface">Password Reset</h2>
              <p class="text-sm text-on-surface-variant">Your password has been updated. You can now sign in with your new password.</p>
              <button
                solexRipple
                type="button"
                (click)="goToLogin()"
            ...
            <div class="text-center space-y-4">
              <div class="w-14 h-14 rounded-full bg-error-container flex items-center justify-center mx-auto">
                <span class="material-symbols-outlined text-on-error-container text-3xl">link_off</span>
              </div>
              <h2 class="text-xl font-bold text-on-surface">Invalid Reset Link</h2>
              <p class="text-sm text-on-surface-variant">This password reset link is invalid or has expired. Please request a new one.</p>
              <button
                solexRipple
                type="button"
                (click)="goToLogin()"
            ...
            <h2 class="text-xl font-bold text-on-surface mb-2">Set New Password</h2>
            <p class="text-sm text-on-surface-variant mb-6">
              Resetting password for <span class="font-medium text-on-surface">{{ email() }}</span>
            </p>
            ...
            <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-4">
              ...
              <div [shakeOnError]="newPasswordInvalid()">
                <label class="block text-sm font-medium text-on-surface-variant mb-1">New Password</label>
                <div class="relative">
                ...
                @if (newPasswordInvalid()) {
                  <p class="text-xs text-error mt-1">
                    @if (form.controls.newPassword.hasError('required')) { Password is required }
                    @else if (form.controls.newPassword.hasError('minlength')) { Must be at least 8 characters }
                  </p>
                }
              </div>
              ...
              <div [shakeOnError]="confirmInvalid()">
                <label class="block text-sm font-medium text-on-surface-variant mb-1">Confirm Password</label>
                <div class="relative">
                  <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-outline">lock_reset</span>
                  <input
                    [type]="showPassword() ? 'text' : 'password'"
...
export class ResetPasswordPageComponent implements OnInit {
  ...
  submit() {
    ...
    this.http.post<{ message: string }>('/api/auth/reset-password', {
      ...
      error: (err: any) => {
        this.isLoading.set(false);
        const msg = err.error?.message || 'Unable to reset password. The link may have expired.';
